 ...
 * @since 3.0.0
 */
EJSC.Chart = EJSC.Drawing.extend({
    /**
    ...
    setAxisBottom: function(axisBottom, apply) {
        ...
        if (!(axisBottom instanceof EJSC.Axis)) {
            axisBottom = new EJSC.LinearAxis(axisBottom);
        }
    ...
    setAxisLeft: function(axisLeft, apply) {
        // Make sure the axis is actually an axis
        if (!(axisLeft instanceof EJSC.Axis)) {
            axisLeft = new EJSC.LinearAxis(axisLeft);
        }
    ...
    setAxisRight: function(axisRight, apply) {
        // Make sure the axis is actually an axis
        if (!(axisRight instanceof EJSC.Axis)) {
            axisRight = new EJSC.LinearAxis(axisRight);
        }
    ...
    setAxisTop: function(axisTop, apply) {
        // Make sure the axis is actually an axis
        if (!(axisTop instanceof EJSC.Axis)) {
            axisTop = new EJSC.LinearAxis(axisTop);
        }
    ...
    addSeries: function(series, redraw) {
        ...
        if (!(series instanceof EJSC.Series)) {
            return null;
        }
    ...
    prepareAxes: function() {
        ...
        EJSC._.forEach(this.sidesList, function(side) {
            ...
            if (!(chart[axis] instanceof EJSC.Axis)) {
                chart[axis] = new EJSC.LinearAxis(chart[axis]);
            }
    ...
    */
    ...
    prepare: function(chart, side) {
        ...
        EJSC._.merge(this, {
            chart: chart,
            side: side,
            orientation: EJSC.Axis.orientations[side]
        });

        // Update the size if needed
        if (EJSC._.isNil(this.size)) {
            this.size = EJSC.Axis.sizes[this.orientation];
        }
    },
 ...
 * @since 3.0.0
 */
EJSC.LinearAxis = EJSC.Axis.extend({
    calculateCross: function(point1, point2, coordinate) {
        // Define some local variables
        var x = coordinate;
        var y = coordinate;
        var x1 = point1.x;
 ...
 * @since 3.0.0
 */
EJSC.LogarithmicAxis = EJSC.Axis.extend({
    // TODO:
    base: 10,
 ...
 * @since 3.0.0
 */
EJSC.DataSeries = EJSC.Class.extend({
    // TODO:
    data: [],
 ...
 * @since 3.0.0
 */
EJSC.PlotSeries = EJSC.Series.extend({
    /**
 ...
 * @since 3.0.0
 */
EJSC.XYSeries = EJSC.PlotSeries.extend([
    EJSC.DataSeries
], {
    dataPoints: {
        x: ['x'],
 ...
 * @since 3.0.0
 */
EJSC.ScatterSeries = EJSC.XYSeries.extend({
    /**
 ...
 * @since 3.0.0
 */
EJSC.LineSeries = EJSC.ScatterSeries.extend({
    /**
 ...
 * @since 3.0.0
 */
EJSC.AreaSeries = EJSC.LineSeries.extend({
    /**
 ...
 * @since 3.0.0
 */
EJSC.BarSeries = EJSC.ScatterSeries.extend({
    /**
